...
function formatSelectedListingLabel(id: string, index: number): string {
  const shortId = id.slice(0, 8)
  return `Advisor ${index + 1} (${shortId})`
}

export default function RequestIntroPageClient() {
                          ...
                          <div
                            ...
                            onKeyDown={(event) => {
                              if (event.key === "Enter" || event.key === " ") {
                                ...
                                toggleAdviceType(type.value)
                              }
                            }}
                            role="button"
                            tabIndex={0}
                            aria-pressed={formData.adviceType.includes(type.value)}
                          >
                            <Checkbox
