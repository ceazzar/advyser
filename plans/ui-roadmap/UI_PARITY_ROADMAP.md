...
## 10) Phase 6 Deep Implementation Review (Merged)

- Date: 2026-02-11
- Reviewer mode: Senior staff engineer implementation review
...
##### 1.1 Prefix-based route checks cause incorrect auth gating and authorization decisions
- Impact:
  - Public routes that share protected prefixes can be treated as protected.
  - Authorization can be incorrectly granted/denied for similarly prefixed routes.
- Proof:
  - Middleware protected check uses `pathname.startsWith(...)`:
    - `/Users/ceazar/Code Base/advyser/site/src/middleware.ts:41`
    - `/Users/ceazar/Code Base/advyser/site/src/middleware.ts:43`
  - Role access check also uses prefix startsWith:
    - `/Users/ceazar/Code Base/advyser/site/src/lib/auth-routing.ts:40`
  - Verified boundary behavior: `'/advisor-resources'.startsWith('/advisor') === true`.
  - Public page exists at:
    - `/Users/ceazar/Code Base/advyser/site/src/app/advisor-resources/page.tsx:91`
- Precise fix:
  - Replace prefix checks with segment-aware matching:
    - `pathname === prefix || pathname.startsWith(prefix + '/')`
  - Apply in both middleware and auth-routing.
  - Add tests for boundary paths (`/advisor-resources`, `/admin-help`) in:
    - `/Users/ceazar/Code Base/advyser/site/src/lib/auth-routing.test.ts`

##### 1.2 `review` RLS allows consumer updates to integrity-sensitive fields
- Impact:
  - Consumers can potentially mutate tenant-targeting or moderation-linked fields on own rows.
  - Review trust/moderation integrity can be weakened.
- Proof:
  - Consumer policies only enforce `consumer_user_id = auth.uid()`:
    - `/Users/ceazar/Code Base/advyser/site/supabase/migrations/20260211210000_phase6_auth_tenancy_rls.sql:643`
    - `/Users/ceazar/Code Base/advyser/site/supabase/migrations/20260211210000_phase6_auth_tenancy_rls.sql:650`
  - `review` schema includes moderation and tenant-linked columns:
    - `/Users/ceazar/Code Base/advyser/database/schema.sql:979`
    - `/Users/ceazar/Code Base/advyser/database/schema.sql:981`
    - `/Users/ceazar/Code Base/advyser/database/schema.sql:993`
  - Seeder demonstrates these fields are writable data columns:
    - `/Users/ceazar/Code Base/advyser/site/scripts/seed-demo-users.mjs:521`
    - `/Users/ceazar/Code Base/advyser/site/scripts/seed-demo-users.mjs:527`
    - `/Users/ceazar/Code Base/advyser/site/scripts/seed-demo-users.mjs:535`
- Precise fix:
  - Tighten review `INSERT/UPDATE` checks to enforce valid engagement linkage (`lead_id`, `business_id`, `listing_id`) for the same consumer.
  - Disallow consumer writes to moderation/system fields via trigger or constrained RPC path.

#### Medium

##### 1.3 `verify-phase6` has false-positive-prone claim-request assertions
- Impact:
  - Gate evidence can pass even when claim-request authorization is broken.
- Proof:
  - Consumer claim assertion expects zero rows (not a positive ownership check):
    - `/Users/ceazar/Code Base/advyser/site/scripts/verify-phase6-rls.mjs:324`
  - Admin check only verifies no query error:
    - `/Users/ceazar/Code Base/advyser/site/scripts/verify-phase6-rls.mjs:333`
  - No deterministic claim-request seed fixture in phase seed script:
    - `/Users/ceazar/Code Base/advyser/site/scripts/seed-demo-users.mjs` (no `claim_request` inserts)
- Precise fix:
  - Seed deterministic claim_request records.
  - Add positive + negative assertions for owner, outsider, advisor-member, and admin with exact expected IDs/counts.

##### 1.4 Migration drops all public policies before recreating a managed subset
- Impact:
  - Existing custom policies can be removed silently, causing security/behavior regressions on established projects.
- Proof:
  - Global policy drop loop:
    - `/Users/ceazar/Code Base/advyser/site/supabase/migrations/20260211210000_phase6_auth_tenancy_rls.sql:165`
    - `/Users/ceazar/Code Base/advyser/site/supabase/migrations/20260211210000_phase6_auth_tenancy_rls.sql:170`
  - Blanket admin policy recreation:
    - `/Users/ceazar/Code Base/advyser/site/supabase/migrations/20260211210000_phase6_auth_tenancy_rls.sql:204`
    - `/Users/ceazar/Code Base/advyser/site/supabase/migrations/20260211210000_phase6_auth_tenancy_rls.sql:214`
- Precise fix:
  - Replace blanket drops with explicit table/policy-name managed updates.
  - Add post-migration policy manifest checks for expected policy set.

#### Low

##### 1.5 Evidence wording overstates cross-tenant coverage breadth
- Impact:
  - Documentation confidence exceeds demonstrated assertion coverage.
- Proof:
  - Broad claim in phase evidence:
    - `/Users/ceazar/Code Base/advyser/plans/phase-6/PHASE_6_GATE_EVIDENCE.md:35`
  - Actual negative tests are limited to select lead/conversation/read + one write spoof path:
    - `/Users/ceazar/Code Base/advyser/site/scripts/verify-phase6-rls.mjs:292`
    - `/Users/ceazar/Code Base/advyser/site/scripts/verify-phase6-rls.mjs:300`
    - `/Users/ceazar/Code Base/advyser/site/scripts/verify-phase6-rls.mjs:311`
- Precise fix:
  - Narrow wording to sampled critical paths, or expand tests to a fuller table-domain matrix.

### 2. Open questions / assumptions

1. Assumed `/advisor-resources` is intended public (if intentionally protected, finding 1.1 severity decreases).
2. Assumed review moderation fields are intended admin/system controlled.
3. Assumed migration should remain safe for pre-existing remote projects with legacy/custom policies.

### 3. Final go/no-go for "Phase 6 complete"

- Decision: **No-go**.
- Reason: High-severity blocking findings remain despite command-level pass.

### 4. Residual risks (even if fixed)

1. Next.js middleware deprecation (`middleware` -> `proxy`) remains and should be scheduled.
2. RLS verification still lacks broad table-by-table mutation/read negative tests for private domains.
3. CI lint warning volume is high (warnings-only gate), which can hide future regressions.
